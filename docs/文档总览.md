...
macOS 打包事故归档见：`docs/Electron-macOS-打包归档-2026-04-12.md`
...
**macOS 打包后双击无界面或很快退出**：优先确认是否通过 `package.json` 预设命令打包/启动。不要从被 IDE 注入环境变量的临时命令直接启动 Electron。详见 `docs/Electron-macOS-打包归档-2026-04-12.md`。
...
└── docs/
    ├── 文档总览.md      # 本文档
    ├── 未来开发计划.md   # 暂缓/待排期的功能想法与归档已下线能力
    ├── Electron-macOS-打包归档-2026-04-12.md
    ├── 架构与修改地图.md
    └── 版本修复归档-2026-04-14.md
```
...
- `docs/架构与修改地图.md`：先看模块职责、关键数据流、常见改动入口与验证矩阵
- `docs/拆分清单.md`：App.tsx / 样式 / gateway 拆分方案与推荐顺序
- `docs/未来开发计划.md`：当前不做、之后再做的功能想法和已下线能力的复用路径
- `CHANGELOG.md`：快速看版本级变化摘要
- `docs/版本修复归档-2026-04-14.md`：查看 2026-04-14 这轮 UI、上传、下载、解压、并发隔离的详细修复记录
- `docs/Electron-macOS-打包归档-2026-04-12.md`：查看 Electron macOS 打包启动问题的完整排障与结论归档

后续如果再出现“历史问题重复出现”的情况，优先先查这两类归档：

- 功能/交互/文件传输相关问题：先看版本修复归档
- 安装/打包/启动相关问题：先看 Electron 打包归档
- 想做但还没排期的能力：先看 `docs/未来开发计划.md`，避免重复讨论同一件事
